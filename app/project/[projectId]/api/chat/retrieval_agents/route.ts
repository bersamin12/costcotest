...
// Define explicit types for message content
type MessageContent = string | Record<string, unknown> | Array<string | Record<string, unknown>>;

// Enhanced validation function with explicit type annotations
const validateMessages = (messages: BaseMessage[]): BaseMessage[] => {
  ...
  return messages.filter(message => {
    ...
    // For array content (which Gemini uses for multimodal)
    ...
    // Default case (undefined, null, etc.)
    return false;
  });
};
...
export async function POST(
  ...
  try {
    ...
    // Log the messages before and after validation to help debugging
    console.log(`Original messages count: ${messages.length}`);
    console.log(`Validated messages count: ${validatedMessages.length}`);
    
    // Additional check to ensure we have valid messages to process
    if (validatedMessages.length === 0) {
      console.log("No valid messages to process after validation");
      return NextResponse.json(
    ...
    if (!returnIntermediateSteps) {
      ...
      try {
        ...
        const eventStream = await agentWithHistory.streamEvents(
          { 
            messages: validatedMessages, // Use validatedMessages here
            history: [] 
          },
          { 
            version: "v2",
